...
const registry = JSON.parse(readFileSync(new URL('../bin/registry.generated.json', import.meta.url), 'utf8'));
const cases = [
  [visionModel, true],
...
test('vision capabilities match only the exact gateway model ID', () => {
  for (const [id, expected] of cases) assert.equal(modelSupportsVision(id), expected, id);
  assert.ok(registry.defaults.models.includes(visionModel));
  const opencode = registry.agents.find(agent => agent.id === 'opencode');
  const models = opencode.env.OPENCODE_CONFIG_CONTENT.$json.provider.subconscious.models;
  assert.equal(models[visionModel].attachment, true);
  assert.deepEqual(models[visionModel].modalities, { input: ['text', 'image'], output: ['text'] });
});

test('generated capability data matches the registry source', () => {
  const source = JSON.parse(readFileSync(new URL('../../agents/registry.json', import.meta.url), 'utf8'));
  assert.deepEqual(registry.modelCapabilities, source.modelCapabilities);
  assert.deepEqual(registry.defaults, source.defaults);
});

test('Unix capability lookup agrees with Windows for exact IDs', { skip: process.platform === 'win32' }, () => {
  const helper = new URL('../bin/runbook/model-capabilities.generated.sh', import.meta.url).pathname;
  for (const [id, expected] of cases) {
    const result = spawnSync('bash', ['-c', 'source "$1"; subc_model_supports_vision "$2"', 'test', helper, id], { encoding: 'utf8' });
